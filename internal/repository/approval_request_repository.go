...
func (r *ApprovalRequestRepository) GetApprovalRequest(ctx context.Context, userID int) ([]*model.ApprovalRequest, error) {
	...
	err := r.db.WithContext(ctx).Where("user_id = ?", userID).Preload("Document").Preload("Document.Category").Preload("Document.File").Preload("Document.User").Preload("Document.User.File").Find(&approvalRequests).Error
	if err != nil {
		return nil, err
	}
	return approvalRequests, nil
